...
namespace TLK.IO.legacy
{
    public class TLKFILE : IDisposable
    {
        ...
        private const string BG_CheckStr = "TLK V1  ";
        ...
        public byte[] GetEntryNoText(int index)
        {
            fs.Seek(EntriseOffset(index), SeekOrigin.Begin);

            return br.ReadBytes((int)ESIZE._stringOffsetOff);
        }
        ...
        {
            byte[] _BlankEntry = new byte[(int)ESIZE.entriseSize];

            ReplaceBytes(
                EntriseOffset(index), 
                (int)ESIZE.entriseSize, 
                _BlankEntry, 
                _BlankEntry.Length);
        }
        ...
        public void RemoveItem(int index)
        {
            ...
            //remove entry
            RemoveBytes(EntriseOffset(index), (int)ESIZE.entriseSize);
        }
        ...
        private bool TLKFileVersionCheck()
        {
            ...
            using (var _fs = new FileStream(filepath, FileMode.Open, FileAccess.Read))
            {
                ...
                switch(chkstr)
                {
                    case BG_CheckStr: result = true; break;
        ...
        {
            var Buff = new byte[(int)(fileLength - (oldIndex + oldLength))];
            int BuffLeng = Buff.Length;
            int readIndex = oldIndex + oldLength;
            int writeIndex = oldIndex + newLength;
